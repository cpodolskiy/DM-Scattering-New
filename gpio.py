import lgpio as sbc
import pigpio
from lgpio import SET_PULL_UP, SET_PULL_DOWN
...
class gpio:
    ...
    def setPUD(self, pin, UD):
        if self._state == 4:
            ...
            if UD == pigpio.PUD_DOWN:
                sbc.gpio_claim_output(self._handle, pin, lFlags=SET_PULL_DOWN)
            else:
                sbc.gpio_claim_output(self._handle, pin, lFlags=SET_PULL_UP)
    ...
    def writePWM(self, pin, speed):
        if self._state == 4:
            ...
            # for f in [1000, 2000, 5000, 10000, 20000, 40000]:
            #     try:
            #         sbc.tx_pwm(self._handle, pin, f, 50) 
            #         print(f"{f} Hz supported")
            #     except sbc.error as e:
            #         print(f"{f} Hz failed: {e}")

            sbc.tx_pwm(self._handle, pin, 6250, float(speed / 480)*100)#converts to percent